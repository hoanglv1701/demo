...
export const NO_ERROR_MSG_CODE: StringOrNumber[] = [];

/** Error message stack to prevent the same error from appearing at the same time */
const errorMsgStack = new Map<StringOrNumber, string>([]);
 ...
 * @param error
 */
export function showErrorMsg(error: Service.RequestError, errorMessageMode: Service.ErrorMessageMode = 'notification') {
  if (!error.msg || NO_ERROR_MSG_CODE.includes(error.code) || hasErrorMsg(error) || errorMessageMode === 'none') return;
 ...
 */
export function handleAxiosError(axiosError: AxiosError, errorMessageMode: Service.ErrorMessageMode = 'message') {
  ...
  const actions: Common.StrategyAction[] = [
    ...
    [
      ...
      Boolean((axiosError.response?.data as Service.BackendErrorResult)?.message),
      () => {
        ...
        if (axiosError.response?.data) {
          // if (
          //   axiosError.request.responseURL.includes(import.meta.env.VITE_CATEGORY_API_URL) &&
          //   axiosError.response.status !== 401 &&
          //   axiosError.response.status !== 403
          // ) {
          //   Object.assign(error, { code: (axiosError.response.data as any)['code'], msg: 'Kết nối danh mục thất bại' });
          // } else if (
          //   axiosError.request.responseURL.includes(import.meta.env.VITE_QTUD_API_URL) &&
          //   axiosError.response.status !== 401 &&
          //   axiosError.response.status !== 403
          // ) {
          //   Object.assign(error, {
          //     code: (axiosError.response.data as any)['code'],
          //     msg: 'Kết nối quản trị ứng dụng thất bại'
          //   });
          // } else {
    ...
    [
      ...
      () => {
        const errorCode: ErrorStatus = (axiosError.response?.status as ErrorStatus) || 'DEFAULT';
        const msg = axiosError.response?.data?.errorDetail || ERROR_STATUS[errorCode];
