...
export class Http {
  ...
  formData<T>(config: AxiosRequestConfig, options?: RequestOptions): Promise<T> {
  ...
  async downloadFile(
    ...
    opt: { fileName?: string; manual?: boolean; saveAs?: boolean; noMessage?: boolean } = {
      manual: false,
      noMessage: false
    }
  ) {
    ...
    if (!opt?.manual) {
      ...
      await downloadByData(response.data, name, opt?.saveAs, opt?.noMessage);
    }
