...
import { REGEX_ALPHABET_CHARACTER_AND_SPECIAL_CHARACTER, REGEX_NUMBER_FORMAT, REGEX_NUMBER_PARSER } from '@/config';
...
export const BaseInputNumber: React.FC<BaseInputNumberProps> = ({
  ...
  function getMaxLengthAttribute(): number | undefined {
    if (maxLength) {
      const maxNumberByMaxLength = getMaxNumber(maxLength);
      return getMaxLength(maxNumberByMaxLength);
    }
    if (max) {
  ...
  function getMaxNumberAttribute(): number | string | undefined {
    ...
    if (maxLength) {
      return getMaxNumber(maxLength);
    }
  }

  function getMaxNumber(maxLength: number): number {
  ...
  return (
    <Label name={`${name}`} label={label} active={hasValue} required={required}>
      <AntInput
        ...
        onChange={setState}
        decimalSeparator='.'
        formatter={(value) => `${value?.toString()}`.replace(REGEX_NUMBER_FORMAT, thousandSeperator)}
        parser={(value) => value?.replace(REGEX_NUMBER_PARSER, '') as unknown as number}
