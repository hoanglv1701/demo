...
import { REGEX_CMND_CCCD, REGEX_NO_SPECIAL_CHARACTER, REGEX_NUMBER, REGEX_NUMBER_OR_CHAR } from '@/config';
...
export const ruleRequired: Rule = {
  // required: true,
  // transform: (value) => {
  //   return isLabelInValueMode(value) ? value.value : isArray(value) ? value[0] : value;
  // },
  validator(_rule, value) {
    const hasValue: boolean = isLabelInValueMode(value)
      ? !!value.value
      : isArray(value)
...
export const ruleKhoangThoiGianNotAllowFutureDate: Rule = {
  validator(_rule, value?: string) {
    ...
    if (!isTuNgayDenNgayFormat) {
      const transformDate = dayjs(transformDateSeperator(value), DATE_VALID_FORMAT, true);
      const isFutureDate = transformDate.diff(dayjs().endOf('day')) >= 0;
...
export const ruleMaxFileSize = (maxFileSize: number): Rule => ({
  validator(_rule, value) {
    ...
    return fileSize <= maxFileSizeToBytes ? Promise.resolve() : Promise.reject();
  },
