...
export function formatToLocalDate(date: dayjs.ConfigType): string {
  return dayjs(date).format(LOCAL_DATE_FORMAT);
}

export function fromStringLocalDate(dateString: string): Date {
  return dayjs(dateString, LOCAL_DATE_FORMAT).toDate();
}

export function defaultDate(date: any): any {
